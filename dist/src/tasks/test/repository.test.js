"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var pg_1 = require("pg");
var tasks_repository_1 = require("../tasks.repository");
var mockClient = {
    query: jest.fn(),
    release: jest.fn()
};
jest.mock('pg', function () {
    var mockPool = {
        connect: jest.fn(function () { return mockClient; }),
        query: jest.fn()
    };
    return { Pool: jest.fn(function () { return mockPool; }) };
});
describe('repository methods', function () {
    var pool;
    beforeEach(function () { return __awaiter(void 0, void 0, void 0, function () {
        return __generator(this, function (_a) {
            pool = new pg_1.Pool();
            return [2];
        });
    }); });
    afterEach(function () {
        jest.clearAllMocks();
    });
    var mockTasks = [];
    describe('getAll', function () {
        test('should return arr', function () { return __awaiter(void 0, void 0, void 0, function () {
            var expected;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        pool.query.mockResolvedValue({ rowCount: 0, rows: [] });
                        return [4, tasks_repository_1.getAll()];
                    case 1:
                        expected = _a.sent();
                        expect(pool.query).toBeCalled();
                        expect(expected).toEqual(mockTasks);
                        return [2];
                }
            });
        }); });
    });
    describe('getByID', function () {
        test('should return one task', function () { return __awaiter(void 0, void 0, void 0, function () {
            var expected;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        pool.query.mockResolvedValue({ rowCountL: 0, rows: [] });
                        return [4, tasks_repository_1.getById(1)];
                    case 1:
                        expected = _a.sent();
                        expect(pool.query).toBeCalled();
                        expect(expected).toEqual(mockTasks);
                        return [2];
                }
            });
        }); });
    });
    test('delByID', function () { return __awaiter(void 0, void 0, void 0, function () {
        var client;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, pool.connect()];
                case 1:
                    client = _a.sent();
                    client.query.mockResolvedValueOnce({ rows: [], rowCount: 1 });
                    return [4, tasks_repository_1.delById(1)];
                case 2:
                    _a.sent();
                    expect(client.query).toHaveBeenCalledWith('BEGIN');
                    expect(client.query).toHaveBeenCalledWith('DELETE FROM education.task WHERE id = $1', [1]);
                    expect(client.query).toHaveBeenCalledWith('COMMIT');
                    expect(client.release).toBeCalled();
                    return [2];
            }
        });
    }); });
    test('update', function () { return __awaiter(void 0, void 0, void 0, function () {
        var client;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, pool.connect()];
                case 1:
                    client = _a.sent();
                    client.query.mockResolvedValueOnce({ rows: [], rowCount: 0 });
                    return [4, tasks_repository_1.update(1, { id: 1, title: 'element', description: 'testEl' })];
                case 2:
                    _a.sent();
                    expect(client.query).toHaveBeenCalledWith('BEGIN');
                    expect(client.query).toHaveBeenCalledWith('UPDATE education.task SET title = $1, description = $2 WHERE ID = 1', ['element', 'testEl']);
                    expect(client.query).toHaveBeenCalledWith('COMMIT');
                    expect(client.release).toBeCalled();
                    return [2];
            }
        });
    }); });
    test('save', function () { return __awaiter(void 0, void 0, void 0, function () {
        var client;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, pool.connect()];
                case 1:
                    client = _a.sent();
                    client.query.mockResolvedValueOnce({ rows: [], rowCount: 0 });
                    return [4, tasks_repository_1.save({ id: 1, title: 'element', description: 'testEl' })];
                case 2:
                    _a.sent();
                    expect(client.query).toHaveBeenCalledWith('BEGIN');
                    expect(client.query).toHaveBeenCalledWith('INSERT INTO education.task (title, description) VALUES ($1, $2)', ['element', 'testEl']);
                    expect(client.query).toHaveBeenCalledWith('COMMIT');
                    expect(client.release).toBeCalled();
                    return [2];
            }
        });
    }); });
});
//# sourceMappingURL=repository.test.js.map